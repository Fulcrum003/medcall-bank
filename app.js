  ...
  if(st==="noendpoint") html+=`<div class="card pad"><div class="faint" style="font-size:13px;line-height:1.5">No group Script connected. Add your Apps Script URL via the manifest <span class="mono">leaderboard</span> field, then refresh.</div></div>`;
    ...
    if(!data.length){ html+=`<div class="empty">No reports yet. They'll appear here as your group submits them.</div>`; }
      ...
      data.reverse().forEach(r=>{
        ...
        html+=`<div class="card pad" style="margin-bottom:8px;border-left:3px solid var(--amber)">
          ...
          <div class="row between" style="font-size:11.5px;margin-top:8px"><span class="faint">by ${who}</span><span class="mono faint">${qid}</span></div>
        </div>`;
      });
    }
  }
...
function viewSettings(){
  ...
  let html=`<div class="fade">
    ...
    <button class="btn btn-ghost" data-action="open-reports-inbox"><svg class="i" viewBox="0 0 24 24"><path d="M4 5h16v11H6l-2 3V5z"/></svg> Open reports inbox</button>
    <div class="faint" style="font-size:12px;margin-top:6px">Reads everyone's submitted reports from your Google Script (needs the doGet <span class="mono">?reports=1</span> branch added + redeploy).</div>
    <div style="height:10px"></div>
    ...
    <div class="empty" style="padding-top:24px" data-action="maint-tap">MedCall · v1.2<br>Storage: ${STORE.kind==='local'?'saved on this device':STORE.kind==='artifact'?'saved (preview)':'this session only'}.</div>
  </div>`;
  return html;
}
...
if(typeof document!=="undefined"){ document.addEventListener("visibilitychange", ()=>{ if(!document.hidden) scheduleReminders(); }); }
