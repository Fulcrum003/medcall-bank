...
function viewTimer(){
  const t=DB.progress.timer||{}, running=!!t.running, sys=t.subject||"General";
  ...
  let html=`<div class="fade">
    ...
    <p class="muted" style="font-size:13.5px;margin-top:2px">Track real study time.${dd!=null?` Exam in <b>${dd>0?dd:0}</b> day${dd===1?"":"s"}.`:""}</p>
    <div class="card pad" style="text-align:center;padding:26px 16px;margin-top:10px">
      <div class="faint" style="font-size:11px;letter-spacing:.12em;text-transform:uppercase">${running?("Studying"+(t.subject?" · "+esc(t.subject):"")):"Ready when you are"}</div>
      <div id="timerDisp" class="mono" style="font-size:46px;font-weight:800;letter-spacing:1px;margin:8px 0">${fmtHMS(sessionElapsed())}</div>
      <button class="btn ${running?"btn-ghost":"btn-primary"}" data-action="${running?"stop-timer":"start-timer"}">${running?"Stop & save":"Start studying"}</button>
    </div>`;
  if(!running){ html+=`<div class="sectlabel">Subject</div><div class="wrapflex">${subs.map(z=>`<button class="chip ${sys===z?"on":""}" data-action="timer-subject" data-s="${esc(z)}">${esc(z)}</button>`).join("")}</div>`; }
...
function viewHome(){
  ...
  html+=`<button class="card pad subj" data-action="start-duel" style="width:100%;margin-bottom:11px;border-left:3px solid var(--purple, #b58fce)">
    <div class="row between"><div class="row" style="gap:11px"><span style="font-size:21px;line-height:1">⚔️</span>
    <div style="text-align:left"><div style="font-weight:700;font-size:15px">Wrong Answer Duel</div>
    <div class="faint" style="font-size:12.5px">${DUELS.length} commonly-confused pairs · pick the stronger answer</div></div></div>
