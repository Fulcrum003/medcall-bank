...
export let DB = {
  ...
  exams:[],
  reports:[],
  settings:{ onboarded:false, account:{mode:"",uid:"",email:"",lastSync:"",autoSync:true}, announcements:[], remoteAnnounce:[], seenAnnounce:[], newPerDay:20, passMark:50, maintainer:false, wallpaper:"ink", theme:"dark", dailyGoal:20, sounds:true, examDate:"", revealOnPick:true, srsGrading:true, notif:{ enabled:false, daily:true, due:true, streak:true, exam:true, time:"19:00", lastFired:{} } }
};
...
export async function syncBank(base){
  ...
  // firebase config travels in the manifest too → accounts work on every device with no per-user setup
  if(mf.firebase !== undefined){ DB.settings.firebaseConfig = mf.firebase || null; save.settings(); }
  // "what's new" notices ride along too, so a release can announce itself with no app update
  if(mf.announcements !== undefined){ DB.settings.announcements = Array.isArray(mf.announcements)? mf.announcements : []; save.settings(); }
  const packs=[], failed=[];
...
let adminToken=null; // Apps Script ADMIN_TOKEN — unlocks the private reports feed. Device-local, never synced.
...
export function render(){
  ...
  else if(App.screen==="banktype") a.innerHTML=viewBankType();
  ...
  else if(App.screen==="welcome") a.innerHTML=viewWelcome();
  else if(App.screen==="account") a.innerHTML=viewAccount();
  ...
  const _DASH=new Set(["home","bank","banksys","banktype","progress","saved","fixes","system","type","reference","mistakes","disputed","redflag","checklist"]);
...
function viewHome(){
  ...
  html+=`<h2 class="serif" style="font-size:24px;font-weight:600">${greet}</h2>
    <p class="muted" style="font-size:13px;margin-top:2px">Pick up where you left off.</p>
    <div style="height:14px"></div>`;
  ...
  { const _sv=Object.values((DB.progress.sessions||{})).filter(x=>x&&x.ctx&&!x.ctx.smart&&poolFor(x.ctx).length).sort((a,b)=>(b.savedAt||0)-(a.savedAt||0));
    if(_sv.length){
      html+=`<div class="rowlist" style="margin-bottom:14px">`;
      _sv.slice(0,3).forEach(x=>{ const _tot=x.total||poolFor(x.ctx).length, _k=esc(JSON.stringify(x.ctx));
        html+=`<button class="rowlink" data-action="resume-session" data-key="${_k}"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:1em;height:1em;vertical-align:-.14em;flex:none;font-size:18px;color:var(--coral)"><path d="M7 4v16l13-8z"/></svg><div class="grow"><div class="rn">Continue · ${esc(x.label||"Session")}</div><div class="rs">Question ${(x.i||0)+1} of ${_tot}</div></div><span class="iconbtn" data-action="drop-session" data-key="${_k}" title="Remove" style="width:28px;height:28px"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:1em;height:1em;vertical-align:-.14em;flex:none;font-size:14px"><path d="M18 6L6 18M6 6l12 12"/></svg></span></button>`;
      });
      html+=`</div>`;
    }
  }

  const tsvg=p=>`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:17px;height:17px">${p}</svg>`;
  const tile=(attrs,icon,label,sub,wide)=>`<button class="tile${wide?' wide':''}" ${attrs}><span class="ti">${icon}</span><div><div class="tl">${label}</div>${sub?`<div class="faint" style="font-size:11.5px;margin-top:2px">${sub}</div>`:''}</div></button>`;
  html+=`<div class="tilegrid">`;
  html+=tile('data-action="start-smart"', tsvg('<path d="M7 4v16l13-8z"/>'), "Smart review", "Today's session — due &amp; new, interleaved", true);
  html+=tile('data-action="nav" data-screen="bank"', tsvg('<path d="M4 19V5a2 2 0 0 1 2-2h13v18H6a2 2 0 0 1-2-2z"/><path d="M19 17H6a2 2 0 0 0-2 2"/>'), "Question bank");
  html+=tile('data-action="nav" data-screen="exam-builder"', tsvg('<rect x="5" y="4" width="14" height="17" rx="2"/><path d="M9 4a2 2 0 0 1 2-2h2a2 2 0 0 1 2 2v1H9zM9 11h6M9 15h4"/>'), "Exam simulation");
  html+=tile('data-action="nav" data-screen="progress"', tsvg('<path d="M3 3v18h18"/><path d="M7 14l3-3 3 3 4-5"/>'), "Progress");
  html+=tile('data-action="open-timer"', tsvg('<circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 3"/>'), "Study timer");
  html+=tile('data-action="nav" data-screen="saved"', tsvg('<path d="M6 4h12v17l-6-4-6 4z"/>'), "Saved");
  html+=tile('data-action="open-duelpick"', tsvg('<path d="M13 2L4 14h6l-1 8 9-12h-6z"/>'), "Wrong-answer duel");
  html+=tile('data-action="start-cram"', tsvg('<path d="M12 3s5 4.5 5 9a5 5 0 0 1-10 0c0-4.5 5-9 5-9z"/>'), "Cram mode");
  html+=tile('data-action="nav" data-screen="leaderboard"', tsvg('<path d="M9 11a4 4 0 1 0 0-8 4 4 0 0 0 0 8zM3 21v-1a5 5 0 0 1 5-5h2a5 5 0 0 1 5 5v1M16 3.5a4 4 0 0 1 0 7.7M21 21v-1a5 5 0 0 0-4-4.9"/>'), "Leaderboard");
  html+=`</div>`;

  const _mc=mistakeIds().length, _dc=disputedIds().length, _rf=redFlagIds().length;
  const frow=(action,icon,label)=>`<button class="rowlink" data-action="${action}">${ic(icon)}<div class="grow"><div class="rn">${label}</div></div>${chev}</button>`;
  let fr="";
  if(_mc>0) fr+=frow("open-mistakes","ti-flame","Fix my mistakes");
  if(_dc>0) fr+=frow("open-disputed","ti-scale","Disputed answers");
  if(_rf>0) fr+=frow("open-redflag","ti-alert-triangle","Red-flag drills");
  fr+=frow("open-checklist","ti-checklist","Before-exam checklist");
  fr+=`<button class="rowlink" data-action="nav" data-screen="trophies"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:1em;height:1em;vertical-align:-.14em;flex:none;font-size:19px;color:var(--muted);width:20px"><path d="M8 21h8M12 17v4M7 4h10v5a5 5 0 0 1-10 0zM7 4H4v2a3 3 0 0 0 3 3M17 4h3v2a3 3 0 0 1-3 3"/></svg><div class="grow"><div class="rn">Achievements</div></div>${chev}</button>`;
  html+=`<div class="sectlabel">Focus</div><div class="rowlist">`+fr+`</div>`;
...
function bankPickSet(sys, ty){ const key=sys+"||"+ty; if(!App.bankPick||App.bankPick.key!==key) App.bankPick={key, system:sys, type:ty, refs:new Set()}; return App.bankPick.refs; }
function viewBankSystem(){
  const sys=App.nav.system;
  const col=(listSystems().find(s=>s.name===sys)||{}).color||"var(--coral)";
  const types=listTypes(sys);
  const get=n=>types.find(t=>t.name===n)||{name:n,total:0,seen:0,due:0};
  const M=get("MCQs"), E=get("EMQs"), SA=types.find(t=>t.name==="Short Answer");
  const card=(T,accent,icon,blurb)=>{
    const pct=T.total?Math.round(T.seen/T.total*100):0, empty=!T.total;
    return `<button class="card pad subj" data-action="${empty?'':'open-bank-type'}" data-type="${esc(T.name)}" style="width:100%;margin-bottom:11px;${empty?'opacity:.45;cursor:default':''};border-left:3px solid ${accent}">
      <div class="row between" style="align-items:flex-start">
        <div style="text-align:left;min-width:0;flex:1">
          <div class="row" style="gap:8px;align-items:center">${icon}<span style="font-weight:800;font-size:17px">${esc(T.name)}</span></div>
          <div class="faint" style="font-size:12.5px;margin-top:3px">${empty? "None for this subject yet" : `${T.total} question${T.total>1?"s":""}`}</div>
        </div>
        ${empty?"":'<span class="mono" style="font-size:12px;color:'+accent+';flex:none;margin-left:10px">open &rarr;</span>'}
      </div>
    </button>`;
  };
  const icoM='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:17px;height:17px;flex:none;color:var(--coral)"><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/></svg>';
  const icoE='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:17px;height:17px;flex:none;color:var(--teal)"><path d="M8 6h13M8 12h13M8 18h13M3 6h.01M3 12h.01M3 18h.01"/></svg>';
  let html=`<div class="fade">
    ...
    <p class="muted" style="font-size:13.5px;margin-top:2px">Two separate question banks — pick a format.</p>
    <div style="height:10px"></div>`;
  html+=card(M,"var(--coral)",icoM,"single-best-answer");
  html+=card(E,"var(--teal)",icoE,"extended matching, by theme");
  if(SA && SA.total) html+=card(SA,"var(--amber)","", "");
  html+=`</div>`;
  return html;
}
function viewBankType(){
  const sys=App.nav.system, ty=App.nav.type;
  const refs=listReferences(sys, ty);
  const sel=bankPickSet(sys, ty);
  const col=ty==="EMQs"?"var(--teal)":"var(--coral)";
  let html=`<div class="fade">
    <button class="btn-sm btn-ghost" data-action="open-bank-system" data-system="${esc(sys)}" style="margin-bottom:14px"><svg class="i" viewBox="0 0 24 24" style="width:15px;height:15px"><path d="M15 18l-6-6 6-6"/></svg> ${esc(sys)}</button>
    <div class="faint mono" style="font-size:11px;letter-spacing:.08em;text-transform:uppercase;color:${col}">${esc(sys)}</div>
    <h2 class="serif" style="font-size:24px;font-weight:600">${esc(ty)}</h2>
    <p class="muted" style="font-size:13.5px;margin-top:2px">${refs.length} source${refs.length!==1?"s":""} · tick the sessions you want, then start one ${ty==="EMQs"?"EMQ":"MCQ"} quiz.</p>
    <div style="height:8px"></div>`;
  if(!refs.length) html+=`<div class="empty">No ${esc(ty)} for this subject yet.</div>`;
  refs.forEach(r=>{
    ...
    html+=`<button class="card pad subj${on?' picked':''}" data-action="bank-pick" data-ref="${esc(r.name)}" style="width:100%;margin-bottom:9px">
      <div class="row between">
        <div style="text-align:left;min-width:0;flex:1">
          ...
          <div class="faint" style="font-size:12px;margin-top:1px">${r.total} Q</div>
        </div>
  ...
  html+=`</div>`;
  if(sel.size){
    let n=0; sel.forEach(r=>{ n+=poolFor({system:sys,type:ty,reference:r}).length; });
    html+=`<div class="selbar"><div class="selbar-in">
      <div style="flex:1"><b>${sel.size}</b> session${sel.size>1?"s":""} · ${n} ${ty==="EMQs"?"EMQ":"MCQ"} question${n!==1?"s":""}</div>
      <button class="btn btn-primary" data-action="bank-start" style="width:auto">Start</button>
    </div></div>`;
  }
  return html;
}
/* ---------- PROGRESS ---------- */
function progressStatsBlock(){
  const allDue=dueCount(), streak=DB.progress.streak?.current||0, totalSeen=Object.keys(DB.progress.questions).length;
  ...
  const chev=`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:1em;height:1em;vertical-align:-.14em;flex:none;font-size:17px;color:var(--faint)"><path d="M9 6l6 6-6 6"/></svg>`;
  let h=`<div class="card pad" style="margin-bottom:14px"><div class="row" style="text-align:center">
      <div style="flex:1"><div class="mono" style="font-size:22px;font-weight:700;color:var(--coral)">${allDue}</div><div class="faint" style="font-size:11px">Due</div></div>
      <div style="flex:1;border-left:1px solid var(--line);border-right:1px solid var(--line)"><div class="mono" style="font-size:22px;font-weight:700">${streak}</div><div class="faint" style="font-size:11px">Streak</div></div>
      <div style="flex:1"><div class="mono" style="font-size:22px;font-weight:700;color:var(--green)">${totalSeen}</div><div class="faint" style="font-size:11px">Seen</div></div></div></div>`;
  h+=`<div class="card pad" style="margin-bottom:14px"><div class="row between" style="gap:14px;flex-wrap:wrap">
      <div class="row" style="gap:11px;flex:1;min-width:180px"><div class="lvlbadge">${lvl}</div><div style="flex:1;min-width:0"><div style="font-weight:600;font-size:14px">Level ${lvl}</div><div class="faint" style="font-size:11.5px">${inLvl} / 200 XP</div><div class="progressbar" style="margin-top:7px"><i style="width:${inLvl/200*100}%"></i></div></div></div>
      <div class="row" style="gap:10px;flex:none">${ringSVG(dpct, dgDone?'var(--green)':'var(--coral)')}<div><div style="font-weight:600;font-size:13.5px">Daily goal</div><div class="faint" style="font-size:11.5px">${dc} / ${goal}${dgDone?' · reached':''}</div></div></div>
    </div></div>`;
  { const _dd=dDay(), _st=studyToday();
    h+=`<button class="card pad" data-action="open-timer" style="margin-bottom:14px;width:100%;text-align:left;cursor:pointer;color:inherit;display:block"><div class="row between"><div class="row" style="gap:11px"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:1em;height:1em;vertical-align:-.14em;flex:none;font-size:19px;color:var(--muted)"><circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 3"/></svg><div>${_dd!=null?`<div style="font-weight:600;font-size:14.5px">${_dd>0?"D-"+_dd:_dd===0?"Exam today":"exam "+(-_dd)+"d ago"}</div><div class="faint" style="font-size:12px">${_dd>0?"to your exam":_dd===0?"good luck":"set a new date"}</div>`:`<div style="font-weight:600;font-size:14.5px">Study timer</div><div class="faint" style="font-size:12px">track real study time</div>`}</div></div><div class="mono" style="color:var(--coral);text-align:right;font-weight:600">${_st>0?fmtHM(_st):"start"}</div></div></button>`; }
  const sugg=computeSuggestions();
  if(sugg.length){
    h+=`<div class="sectlabel">What should I study?</div><div class="rowlist" style="margin-bottom:14px">`;
    sugg.forEach(s=>{
...
export function gradeCurrent(g){
  ...
  if(g==="auto") g=(correctLabel(q)===s.selected)?"good":"again";
...
function viewQuiz(){
  ...
  } else if(DB.settings.srsGrading===false){
    foot=(q.type==="sa"||singleAnswer)
      ? `<button class="btn btn-ghost" data-action="grade" data-grade="again" style="flex:1">Missed it</button><button class="btn btn-primary" data-action="grade" data-grade="good" style="flex:1">Got it</button>`
      : `<button class="btn btn-primary" data-action="grade" data-grade="auto" style="flex:1">Next question</button>`;
  } else {
...
async function fetchReports(){
  ...
  try{
    const u=ep+(ep.indexOf("?")>=0?"&":"?")+"reports=1&cb="+Date.now()+(adminToken?"&token="+encodeURIComponent(adminToken):"");
    const r=await fetch(u,{cache:"no-store"}); if(!r.ok) throw new Error(r.status);
    const data=await r.json();
    if(data && data.error==="unauthorized"){ App.inboxState="unauthorized"; if(App.screen==="reportsinbox") render(); return; }
...
function viewReportsInbox(){
  ...
  else if(st==="unauthorized") html+=`<div class="card pad" style="border-left:3px solid var(--amber)"><div style="font-size:13px;line-height:1.55">The Script refused the request. Reports are now private — set <span class="mono">ADMIN_TOKEN</span> in the Apps Script (Project Settings → Script Properties), then add the same value here via <b>Settings → Maintainer tools → reports token</b>.</div></div>`;
...
function viewSettings(){
  ...
  let html=`<div class="fade">
    ...
    <div class="row between card pad" style="margin-top:8px">
      <div><b>Flashcard grading</b><div class="faint" style="font-size:12.5px">Rate yourself Again · Hard · Good · Easy after each answer (off = one Next button, graded automatically)</div></div>
      <button class="switch ${DB.settings.srsGrading!==false?'on':''}" data-action="set-srsgrading" aria-label="Toggle flashcard grading"><span class="knob"></span></button>
    </div>
    ...
    <div class="card pad">
      ...
      ${announceComposer()}
      <div style="height:14px"></div>
      <button class="btn btn-ghost" data-action="set-admintoken">${adminToken?"Replace":"Add"} reports token (this device)</button>
      ${adminToken?`<div class="faint" style="font-size:12px;text-align:center;margin-top:8px">Reports token set · <button class="reportlink" data-action="clear-admintoken">remove</button></div>`:`<div class="faint" style="font-size:12px;text-align:center;margin-top:8px">Needed to read the reports inbox after hardening the Apps Script</div>`}
      <div style="height:10px"></div>
...
/* ============================================================
   ACCOUNTS & CROSS-DEVICE SYNC   (optional — Firebase)
   ============================================================
   Guests keep everything on-device exactly as before. Creating an
   account additionally syncs progress across that user's devices.

   The Firebase config travels in manifest.json (same trick as the
   leaderboard endpoint) so every device picks it up automatically:
       "firebase": { "apiKey":"…", "authDomain":"…", "projectId":"…", "appId":"…" }

   The SDK is loaded lazily with dynamic import() the first time an
   account is actually used, so guests never download it and the app
   still starts offline and from file://.
   ============================================================ */

export function acct(){
  if(!DB.settings.account || typeof DB.settings.account!=="object"){
    DB.settings.account = { mode:"", uid:"", email:"", lastSync:"", autoSync:true };
  }
  return DB.settings.account;
}
export function isSignedIn(){ const a=acct(); return a.mode==="account" && !!a.uid; }
export function fbConfig(){ const c=DB.settings.firebaseConfig; return (c && c.apiKey && c.projectId) ? c : null; }

/* ---------- pure merge helpers (unit-tested, no network) ---------- */

function histSig(h){
  return [h && h.date||"", (h&&h.answer!=null)?h.answer:"", (h&&h.correct)?1:0,
          (h&&h.grade)||"", (h&&h.confidence!=null)?h.confidence:""].join("|");
}

// Two devices may each have answered the same question offline. History is the
// source of truth: union it, then derive the counters from it. Never lose an attempt.
export function mergeQuestionProgress(a, b){
  if(!a && !b) return null;
  if(!a) return JSON.parse(JSON.stringify(b));
  if(!b) return JSON.parse(JSON.stringify(a));
  const ha = Array.isArray(a.history)? a.history : [];
  const hb = Array.isArray(b.history)? b.history : [];
  const seenSig = new Set(); const hist = [];
  ha.concat(hb).forEach(h=>{
    if(!h || typeof h!=="object") return;
    const s = histSig(h); if(seenSig.has(s)) return;
    seenSig.add(s); hist.push(h);
  });
  hist.sort((x,y)=> String(x.date||"").localeCompare(String(y.date||"")));
  const aSeen = String(a.lastSeen||""), bSeen = String(b.lastSeen||"");
  const newer = bSeen > aSeen ? b : a;
  const older = newer===b ? a : b;
  // Imported backups can carry {seen:2} with no history — max() so those never shrink.
  const seen    = Math.max(hist.length, +a.seen||0, +b.seen||0);
  const correct = Math.min(seen, Math.max(hist.filter(h=>h&&h.correct).length, +a.correct||0, +b.correct||0));
  return {
    seen, correct, history: hist,
    marked: !!newer.marked,
    lastResult: newer.lastResult || older.lastResult || null,
    lastSeen: aSeen > bSeen ? a.lastSeen : b.lastSeen,
    srs: newer.srs || older.srs || null
  };
}

// Per-date counters (xp, study seconds) take the MAX for a given day rather than
// summing, so re-syncing the same day repeatedly can never inflate totals.
function mergeDateMap(a, b){
  const out = Object.assign({}, a||{});
  const r = b||{};
  Object.keys(r).forEach(d=>{ out[d] = Math.max(+out[d]||0, +r[d]||0); });
  return out;
}
...
export function mergeProgress(local, remote){
  ...
  Object.keys(rq).forEach(id=>{ out.questions[id] = mergeQuestionProgress(out.questions[id], rq[id]); });
  out.xpLog   = mergeDateMap(L.xpLog,   R.xpLog);
  out.timeLog = mergeDateMap(L.timeLog, R.timeLog);
  out.xp      = Math.max(+L.xp||0, +R.xp||0);
  const cl = Object.assign({}, L.checklist||{});
  Object.keys(R.checklist||{}).forEach(k=>{ if(R.checklist[k]) cl[k]=R.checklist[k]; });
  out.checklist = cl;
  const ls = L.streak||{}, rs = R.streak||{};
  const streakBase = String(rs.lastStudied||"") > String(ls.lastStudied||"") ? rs : ls;
  out.streak = Object.assign({current:0,lastStudied:null}, streakBase);
  out.streak.current = Math.max(+ls.current||0, +rs.current||0);
  // resume marker: whichever device studied most recently
  if(R.resume && (!L.resume || String(R.resume.date||"") > String(L.resume.date||""))) out.resume = R.resume;
  return out;
}

// Arrays of records (exams, reports) — union, preferring local on id collision.
export function mergeRecords(localArr, remoteArr, idKey){
  const out = Array.isArray(localArr) ? localArr.slice() : [];
  const seen = new Set(out.map(x=> x && (x[idKey] || JSON.stringify(x))));
  (Array.isArray(remoteArr)?remoteArr:[]).forEach(x=>{
    if(!x) return;
    const k = x[idKey] || JSON.stringify(x);
    if(!seen.has(k)){ seen.add(k); out.push(x); }
  });
  return out;
}

// Device-specific preferences (theme, wallpaper, maintainer, tokens) deliberately
// do NOT travel between devices. Only study preferences do.
export const SYNCED_SETTINGS = ["newPerDay","passMark","dailyGoal","examDate","revealOnPick","srsGrading","displayName"];

// Firestore caps a document at 1 MiB. Full answer history for a few thousand
// questions can approach that, so trim to the most recent N per question for the
// uploaded copy — SRS state and counters are preserved, only old history rows drop.
export function buildSyncPayload(db, histCap){
  const cap = histCap || 20;
  const prog = JSON.parse(JSON.stringify(db.progress||{}));
  Object.keys(prog.questions||{}).forEach(id=>{
    const q = prog.questions[id];
    if(q && Array.isArray(q.history) && q.history.length > cap) q.history = q.history.slice(-cap);
  });
  const st = {};
  SYNCED_SETTINGS.forEach(k=>{ if(db.settings && db.settings[k]!==undefined) st[k]=db.settings[k]; });
  return { progress: prog, exams: db.exams||[], settings: st, updatedAt: new Date().toISOString(), schema: 1 };
}

function safeParse(str, fallback){ try{ const v=JSON.parse(str); return (v===null||v===undefined)?fallback:v; }catch(e){ return fallback; } }

// Firestore refuses some nested shapes ("Property progress contains an invalid
// nested entity"). Sending JSON strings sidesteps every one of those rules.
export function serializePayload(p){
  return { progressJson: JSON.stringify(p.progress||{}),
           examsJson:    JSON.stringify(p.exams||[]),
           settingsJson: JSON.stringify(p.settings||{}),
           updatedAt:    p.updatedAt || new Date().toISOString(),
           schema:       2 };
}

/* ---------- Firebase adapter (lazy) ---------- */

const FB = { app:null, auth:null, db:null, mod:null };
const FB_VER = "10.12.2";

async function fbLoad(){
  if(FB.mod) return FB.mod;
  const cfg = fbConfig();
  if(!cfg) throw new Error("Accounts are not configured for this app yet.");
  ...
  FB.app  = appM.getApps && appM.getApps().length ? appM.getApp() : appM.initializeApp(cfg);
  FB.auth = authM.getAuth(FB.app);
  FB.db   = fsM.getFirestore(FB.app);
  FB.mod  = { appM, authM, fsM };
  return FB.mod;
}

export async function accountSignUp(email, password){
  const { authM } = await fbLoad();
  const cred = await authM.createUserWithEmailAndPassword(FB.auth, email, password);
  const a = acct(); a.mode="account"; a.uid=cred.user.uid; a.email=cred.user.email||email;
  DB.settings.onboarded = true; save.settings();
  await syncNow(true);
  return a;
}
export async function accountSignIn(email, password){
  const { authM } = await fbLoad();
  const cred = await authM.signInWithEmailAndPassword(FB.auth, email, password);
  const a = acct(); a.mode="account"; a.uid=cred.user.uid; a.email=cred.user.email||email;
  DB.settings.onboarded = true; save.settings();
  await syncNow(true);
  return a;
}
export async function accountReset(email){
  const { authM } = await fbLoad();
  await authM.sendPasswordResetEmail(FB.auth, email);
}
export async function accountSignOut(){
  try{ const { authM } = await fbLoad(); await authM.signOut(FB.auth); }catch(e){}
  const a = acct(); a.mode="guest"; a.uid=""; a.email=""; a.lastSync="";
  save.settings();
}
...
export let syncState = { busy:false, error:"", last:"" };

export function scheduleSync(delay){
  if(!isSignedIn() || acct().autoSync===false) return;
  clearTimeout(_syncT);
  _syncT = setTimeout(()=>{ syncNow(true); }, delay==null?6000:delay);
}

export async function syncNow(quiet){
  if(!isSignedIn()){ if(!quiet) toast("You're using a guest account"); return false; }
  if(_syncing) return false;
  if(typeof navigator!=="undefined" && navigator.onLine===false){
    if(!quiet) toast("Offline — will sync when you're back online");
    return false;
  }
  _syncing=true; syncState.busy=true; syncState.error="";
  if(!quiet && typeof render==="function") try{ render(); }catch(e){}
  try{
    const { fsM } = await fbLoad();
    const ref = fsM.doc(FB.db, "users", acct().uid);
    const snap = await fsM.getDoc(ref);
    if(snap.exists()){
      const remote = snap.data()||{};
      const rProg  = remote.progressJson ? safeParse(remote.progressJson, {}) : (remote.progress||{});
      const rExams = remote.examsJson    ? safeParse(remote.examsJson, [])    : (remote.exams||[]);
      const rSet   = remote.settingsJson ? safeParse(remote.settingsJson, {}) : (remote.settings||{});
      DB.progress = mergeProgress(DB.progress, rProg);
      DB.exams    = mergeRecords(DB.exams, rExams, "id");
      SYNCED_SETTINGS.forEach(k=>{
        if(rSet && rSet[k]!==undefined && DB.settings[k]===undefined) DB.settings[k]=rSet[k];
      });
      save.progress(); save.exams();
    }
    let payload = serializePayload(buildSyncPayload(DB, 20));
    if(payload.progressJson.length > 700000) payload = serializePayload(buildSyncPayload(DB, 6));
    await fsM.setDoc(ref, payload);
    const a=acct(); a.lastSync=new Date().toISOString(); save.settings();
    syncState.last=a.lastSync;
    if(!quiet) toast("Synced ✓");
    return true;
  }catch(e){
    syncState.error = (e && e.message) ? e.message : "Sync failed";
    if(!quiet) toast("Sync failed — "+syncState.error);
    return false;
  }finally{
    _syncing=false; syncState.busy=false;
    if(typeof render==="function") try{ if(App.screen==="account"||App.screen==="settings") render(); }catch(e){}
  }
}


export function acctErrMsg(err){
  const c=(err && (err.code||err.message)) ? String(err.code||err.message) : "";
  if(c.indexOf("email-already-in-use")>=0) return "That email already has an account — switch to Sign in.";
  if(c.indexOf("invalid-email")>=0)        return "That doesn't look like a valid email address.";
  if(c.indexOf("weak-password")>=0)        return "Password is too weak — use at least 6 characters.";
  if(c.indexOf("wrong-password")>=0 || c.indexOf("invalid-credential")>=0 || c.indexOf("user-not-found")>=0)
                                           return "Email or password is incorrect.";
  if(c.indexOf("too-many-requests")>=0)    return "Too many attempts — wait a minute and try again.";
  if(c.indexOf("network")>=0)              return "No connection — check your internet and try again.";
  if(c.indexOf("operation-not-allowed")>=0) return "Email sign-in isn't enabled in Firebase yet (Authentication \u2192 Sign-in method \u2192 Email/Password).";
  if(c.indexOf("permission-denied")>=0 || c.indexOf("insufficient")>=0)
                                           return "The database rules haven't been published yet, so syncing is blocked.";
  if(c.indexOf("unavailable")>=0)          return "Can't reach the sync service \u2014 check your connection and try again.";
  if(c.indexOf("not configured")>=0)       return "Accounts aren't switched on for this build yet.";
  return c || "Something went wrong — try again.";
}


/* ============================================================
   ANNOUNCEMENTS  ("what's new" notices)
   ============================================================
   Notices are authored in manifest.json, so shipping one needs no app
   release. Each is shown once per device; dismissals are stored locally
   and deliberately NOT synced — seeing it on your phone shouldn't hide it
   on your laptop before you've read it there.
   ============================================================ */

export function isAnnouncement(a){
  return !!(a && typeof a==="object" && typeof a.id==="string" && a.id
            && typeof a.title==="string" && a.title.trim());
}

export function pendingAnnounce(){
  const seen = DB.settings.seenAnnounce || [];
  const fromManifest = Array.isArray(DB.settings.announcements) ? DB.settings.announcements : [];
  const fromScript   = Array.isArray(DB.settings.remoteAnnounce) ? DB.settings.remoteAnnounce : [];
  const byId = {};
  fromManifest.concat(fromScript).forEach(a=>{ if(a && a.id) byId[a.id] = a; });  // script wins on id clash
  const all = Object.keys(byId).map(k=>byId[k]);
  return all.filter(a => isAnnouncement(a) && seen.indexOf(a.id) < 0)
            .sort((a,b)=> String(b.date||"").localeCompare(String(a.date||"")));
}
export function markAnnounceSeen(id){
  if(!id) return;
  const seen = DB.settings.seenAnnounce || [];
  if(seen.indexOf(id) < 0){ seen.push(id); DB.settings.seenAnnounce = seen; save.settings(); }
}
// A brand-new user shouldn't be told "what's new" about an app they've never seen.
export function markAllAnnounceSeen(){
  const all = (Array.isArray(DB.settings.announcements)?DB.settings.announcements:[])
    .concat(Array.isArray(DB.settings.remoteAnnounce)?DB.settings.remoteAnnounce:[]);
  DB.settings.seenAnnounce = all.map(a=>a && a.id).filter(Boolean);
  save.settings();
}

export function announceCard(){
  const a = pendingAnnounce()[0];
  if(!a) return "";
  const act = a.action && a.action.label && a.action.screen
    ? `<button class="btn-sm btn-primary" data-action="announce-act" data-screen="${esc(a.action.screen)}" data-id="${esc(a.id)}" style="width:auto">${esc(a.action.label)}</button>`
    : "";
  return `<div class="railcard" style="margin-bottom:16px;border-color:var(--teal)">
    <div class="row" style="gap:12px;align-items:flex-start">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:1em;height:1em;vertical-align:-.14em;flex:none;font-size:19px;color:var(--teal)"><path d="M3 11l18-8-8 18-2-7-8-3z"/></svg>
      <div style="flex:1">
        <div style="font-weight:600;font-size:14.5px">${esc(a.title||"What's new")}</div>
        <div class="faint" style="font-size:13px;margin-top:3px;line-height:1.55">${esc(a.body||"")}</div>
        <div class="row" style="gap:8px;margin-top:11px">${act}
          <button class="btn-sm btn-ghost" data-action="announce-ok" data-id="${esc(a.id)}" style="width:auto">Got it</button>
        </div>
      </div>
    </div></div>`;
}

// Settings → so a notice can be re-read after it's dismissed
export function announceHistory(){
  const all = (Array.isArray(DB.settings.announcements)?DB.settings.announcements:[])
    .concat(Array.isArray(DB.settings.remoteAnnounce)?DB.settings.remoteAnnounce:[]);
  const valid = all.filter(isAnnouncement);
  if(!valid.length) return "";
  const rows = valid.slice().sort((a,b)=> String(b.date||"").localeCompare(String(a.date||"")))
    .map(a=>`<div style="padding:11px 0;border-top:1px solid var(--line-soft)">
      <div class="row between" style="font-size:13.5px"><b>${esc(a.title||"")}</b><span class="faint mono" style="font-size:11px">${esc(a.date||"")}</span></div>
      <div class="faint" style="font-size:12.5px;margin-top:3px;line-height:1.5">${esc(a.body||"")}</div>
    </div>`).join("");
  return `<div class="sectlabel">What's new</div><div class="card pad" style="padding:4px 14px">${rows}</div>`;
}


// Notices published from inside the app live in the Apps Script, so shipping one
// needs neither a repo edit nor an app release. Read is public; writing needs the token.
export async function fetchAnnouncements(){
  const ep = DB.settings.groupEndpoint; if(!ep) return;
  try{
    const u = ep + (ep.indexOf("?")>=0?"&":"?") + "announce=1&cb=" + Date.now();
    const r = await fetch(u,{cache:"no-store"}); if(!r.ok) return;
    const data = await r.json();
    // Guard: an older deployed Script ignores ?announce=1 and returns the
    // leaderboard instead. Only keep things that actually look like notices.
    if(Array.isArray(data)){
      const clean = data.filter(isAnnouncement);
      if(clean.length !== data.length && !clean.length) return;   // wrong endpoint — keep what we have
      DB.settings.remoteAnnounce = clean; save.settings();
    }
  }catch(e){ /* offline — keep whatever we already have */ }
}

export async function publishAnnouncement(a){
  const ep = DB.settings.groupEndpoint;
  if(!ep) throw new Error("No group Script connected");
  if(!adminToken) throw new Error("Add your reports token first (Maintainer tools)");
  if(!a || !a.title) throw new Error("Give the notice a title");
  const payload = { type:"announce", token:adminToken,
    annId: a.id || ("a"+Date.now()), date: a.date || today(),
    title: a.title, body: a.body||"" };
  if(a.actionLabel && a.actionScreen){ payload.actionLabel=a.actionLabel; payload.actionScreen=a.actionScreen; }
  ...
  if(j && j.ok===false) throw new Error(j.err||"rejected");
...
export async function removeAnnouncement(id){
  const ep = DB.settings.groupEndpoint;
  if(!ep || !adminToken) throw new Error("Not configured");
  const r = await fetch(ep,{method:"POST",headers:{"Content-Type":"text/plain;charset=utf-8"},
    body:JSON.stringify({type:"announce", token:adminToken, remove:id})});
  if(!r.ok) throw new Error("Script returned "+r.status);
  await fetchAnnouncements();
  return true;
}

// Maintainer composer — lives in Settings under Maintainer tools.
export function announceComposer(){
  const live = Array.isArray(DB.settings.remoteAnnounce) ? DB.settings.remoteAnnounce : [];
  const rows = live.filter(isAnnouncement).slice().reverse().map(a=>`<div class="row between" style="padding:9px 0;border-top:1px solid var(--line-soft)">
      <div style="flex:1;min-width:0"><div style="font-size:13px;font-weight:600">${esc(a.title||"")}</div>
      <div class="faint mono" style="font-size:11px">${esc(a.date||"")} · ${esc(a.id||"")}</div></div>
      <button class="reportlink" data-action="announce-del" data-id="${esc(a.id)}">remove</button>
    </div>`).join("");
  return `<div class="sectlabel">Post an announcement</div>
  <div class="card pad">
    <div class="faint" style="font-size:12.5px;line-height:1.5;margin-bottom:10px">
      Everyone sees this at the top of Home until they dismiss it. Published through the Script — no file editing.
    </div>
    <input id="annTitle" class="inp" placeholder="Title — e.g. New paediatrics questions added" style="margin-bottom:8px">
    <textarea id="annBody" class="inp" rows="3" placeholder="A sentence or two explaining what changed." style="margin-bottom:8px"></textarea>
    <div class="row" style="gap:8px;margin-bottom:8px">
      <input id="annBtn" class="inp" placeholder="Button label (optional)" style="flex:1">
      <select id="annScreen" class="inp" style="flex:1">
    ...
    <button class="btn btn-primary" data-action="announce-publish">Publish to everyone</button>
    <div id="annErr" style="color:var(--amber);font-size:12.5px;margin-top:9px;line-height:1.5"></div>
    ${rows?`<div style="margin-top:12px"><div class="faint" style="font-size:12px;margin-bottom:2px">Live now (${live.filter(isAnnouncement).length})</div>${rows}</div>`:""}
  </div>`;
}
...
export function viewWelcome(){
  const configured = !!fbConfig();
  return `<div class="fade" style="max-width:520px;margin:0 auto;padding-top:6vh">
    <div style="text-align:center;margin-bottom:26px">
      <div class="serif" style="font-size:30px;font-weight:600;letter-spacing:-.4px">MedRecall</div>
      <div class="faint" style="font-size:14px;margin-top:8px;line-height:1.55">
        Your question bank, spaced repetition and exam simulator.
      </div>
    </div>

    <div class="card pad" style="margin-bottom:12px">
      <div style="font-size:15px;font-weight:600;margin-bottom:4px">Create an account</div>
      <div class="faint" style="font-size:13px;line-height:1.55;margin-bottom:12px">
        Keeps your progress, streak and exam history backed up and ${configured?"synced automatically across your phone, tablet and laptop":"ready to sync across devices"}.
      </div>
      <button class="btn btn-primary" data-action="nav" data-screen="account" data-mode="signup">Create account</button>
      <div style="height:8px"></div>
      <button class="btn btn-ghost" data-action="nav" data-screen="account" data-mode="signin">I already have an account</button>
    </div>

    <div class="card pad">
      <div style="font-size:15px;font-weight:600;margin-bottom:4px">Continue as guest</div>
      <div class="faint" style="font-size:13px;line-height:1.55;margin-bottom:12px">
        Everything works and stays on this device. Your progress won't sync — you can create an account later from Settings without losing anything.
      </div>
      <button class="btn btn-ghost" data-action="acct-guest">Continue as guest</button>
    </div>

    ${configured?"":`<div class="faint" style="font-size:12px;text-align:center;margin-top:16px;line-height:1.5">
      Accounts aren't switched on for this build yet — guest mode has every study feature.</div>`}
  </div>`;
}

export function viewAccount(){
  const a = acct();
  const mode = App.acctMode || "signup";
  const configured = !!fbConfig();
  const back = `<button class="btn-sm btn-ghost" data-action="nav" data-screen="${DB.settings.onboarded?"settings":"welcome"}" style="margin-bottom:14px"><svg class="i" viewBox="0 0 24 24" style="width:15px;height:15px"><path d="M15 18l-6-6 6-6"/></svg> Back</button>`;

  if(isSignedIn()){
    const last = a.lastSync ? new Date(a.lastSync).toLocaleString() : "not yet";
    return `<div class="fade">${back}
      <h2 class="serif" style="font-size:24px;font-weight:600">Account</h2>
      <div class="card pad" style="margin-top:12px">
        <div class="row between"><div><b>${esc(a.email||"Signed in")}</b>
          <div class="faint" style="font-size:12.5px">Progress syncs automatically</div></div>
          <span class="pill" style="border-color:var(--green);color:var(--green)">Synced</span></div>
      </div>
      <div class="row between card pad" style="margin-top:8px">
        <div><b>Automatic sync</b><div class="faint" style="font-size:12.5px">After each session and on reopening</div></div>
        <button class="iconbtn" data-action="acct-autosync">${a.autoSync===false?"Off":"On"}</button>
      </div>
      <div class="card pad" style="margin-top:8px">
        <div class="faint" style="font-size:12.5px;margin-bottom:10px">Last synced: ${esc(last)}</div>
        <button class="btn btn-primary" data-action="acct-sync" ${syncState.busy?"disabled":""}>${syncState.busy?"Syncing…":"Sync now"}</button>
        ${syncState.error?`<div style="color:var(--amber);font-size:12.5px;margin-top:10px;line-height:1.5">${esc(syncState.error)}</div>`:""}
      </div>
      <div style="height:14px"></div>
      <button class="btn btn-ghost" data-action="acct-signout" style="border-color:#6b541f;color:var(--amber)">Sign out on this device</button>
      <div class="faint" style="font-size:12px;margin-top:10px;line-height:1.5">Signing out leaves your progress on this device and stops syncing.</div>
    </div>`;
  }
  ...
  if(!configured){
    return `<div class="fade">${back}
      <h2 class="serif" style="font-size:24px;font-weight:600">Account</h2>
      <div class="card pad" style="margin-top:12px">
        <div class="faint" style="font-size:13px;line-height:1.6">Accounts aren't switched on for this build yet. You're using guest mode — every study feature works and your progress is saved on this device.</div>
      </div></div>`;
  }

  const tab = (id,label)=>`<button class="btn-sm ${mode===id?"btn-primary":"btn-ghost"}" data-action="acct-mode" data-mode="${id}" style="flex:1">${label}</button>`;
  return `<div class="fade">${back}
    <h2 class="serif" style="font-size:24px;font-weight:600">${mode==="signup"?"Create account":"Sign in"}</h2>
    <div class="row" style="gap:8px;margin:14px 0">${tab("signup","Create account")}${tab("signin","Sign in")}</div>
    <div class="card pad">
      <label class="faint" style="font-size:12.5px">Email</label>
      <input id="acctEmail" type="email" autocomplete="email" class="inp" style="width:100%;margin:6px 0 12px" placeholder="you@example.com">
      <label class="faint" style="font-size:12.5px">Password</label>
      <input id="acctPass" type="password" autocomplete="${mode==="signup"?"new-password":"current-password"}" class="inp" style="width:100%;margin:6px 0 4px" placeholder="At least 6 characters">
      <div class="faint" style="font-size:12px;margin-bottom:12px">${mode==="signup"?"Use any email — it's only used to recover your account.":"&nbsp;"}</div>
      <button class="btn btn-primary" data-action="acct-submit">${mode==="signup"?"Create account & sync":"Sign in & sync"}</button>
      ${mode==="signin"?`<div style="height:8px"></div><button class="btn btn-ghost" data-action="acct-reset">Forgot password</button>`:""}
      <div id="acctErr" style="color:var(--amber);font-size:12.5px;margin-top:10px;line-height:1.5"></div>
    </div>
    <div class="faint" style="font-size:12px;margin-top:12px;line-height:1.55">
      Your existing progress on this device is kept and merged into the account — nothing is lost.
    </div>
  </div>`;
}
...
export function accountSettingsCard(){
  const a = acct();
  if(isSignedIn()){
    const last = a.lastSync ? new Date(a.lastSync).toLocaleDateString() : "not yet";
    return `<div class="sectlabel">Account</div>
    <div class="row between card pad" data-action="nav" data-screen="account" style="cursor:pointer">
      <div><b>${esc(a.email||"Signed in")}</b><div class="faint" style="font-size:12.5px">Syncing · last ${esc(last)}</div></div>
      <svg class="i" viewBox="0 0 24 24" style="width:16px;height:16px"><path d="M9 6l6 6-6 6"/></svg>
    </div>`;
  }
  return `<div class="sectlabel">Account</div>
    <div class="row between card pad" data-action="nav" data-screen="account" style="cursor:pointer">
      <div><b>Guest</b><div class="faint" style="font-size:12.5px">Progress stays on this device — tap to sync across devices</div></div>
      <svg class="i" viewBox="0 0 24 24" style="width:16px;height:16px"><path d="M9 6l6 6-6 6"/></svg>
    </div>`;
}
...
document.body.addEventListener("click", async e=>{
  ...
  if(a==="set-admintoken"){ const v=(prompt("Paste the ADMIN_TOKEN you set in the Apps Script (Script Properties). Stored on this device only:")||"").trim(); if(v){ adminToken=v; wsSet("medrecall:admintoken:v1", v); render(); toast("Reports token saved"); if(App.screen==="reportsinbox") fetchReports(); } return; }
  if(a==="clear-admintoken"){ adminToken=null; wsSet("medrecall:admintoken:v1", null); render(); toast("Reports token removed"); return; }
  ...
  /* ---- accounts ---- */
  if(a==="announce-publish"){
    const el=$("annErr"); if(el) el.textContent="";
    const body={ title:($("annTitle")?.value||"").trim(), body:($("annBody")?.value||"").trim(),
                 actionLabel:($("annBtn")?.value||"").trim(), actionScreen:($("annScreen")?.value||"") };
    t.disabled=true; const lbl=t.textContent; t.textContent="Publishing…";
    try{ await publishAnnouncement(body); toast("Announcement published ✓"); render(); }
    catch(err){ t.disabled=false; t.textContent=lbl; const e2=$("annErr"); if(e2) e2.textContent=(err&&err.message)||"Couldn't publish"; }
    return;
  }
  if(a==="announce-del"){
    if(!confirm("Remove this announcement for everyone?")) return;
    try{ await removeAnnouncement(t.dataset.id); toast("Removed"); render(); }catch(err){ toast("Couldn't remove"); }
    return;
  }
  if(a==="announce-ok"){ markAnnounceSeen(t.dataset.id); render(); return; }
  if(a==="announce-act"){ markAnnounceSeen(t.dataset.id); const sc=t.dataset.screen; if(sc==="account") App.acctMode="signup"; App.screen=sc; render(); return; }
  if(a==="acct-guest"){
    const ac=acct(); if(!ac.mode) ac.mode="guest";
    DB.settings.onboarded=true; markAllAnnounceSeen(); save.settings();
    App.screen="home"; render(); toast("You're in — progress saves on this device");
    return;
  }
  if(a==="acct-mode"){ App.acctMode=t.dataset.mode||"signup"; render(); return; }
  if(a==="acct-autosync"){ const ac=acct(); ac.autoSync = ac.autoSync===false; save.settings(); render(); if(ac.autoSync) syncNow(true); return; }
  if(a==="acct-sync"){ await syncNow(false); return; }
  if(a==="acct-signout"){
    if(!confirm("Sign out on this device? Your progress stays here and stops syncing.")) return;
    await accountSignOut(); App.screen="settings"; render(); toast("Signed out");
    return;
  }
  if(a==="acct-reset"){
    const em=($("acctEmail")?.value||"").trim();
    if(!em){ const el=$("acctErr"); if(el) el.textContent="Enter your email first, then tap Forgot password."; return; }
    try{ await accountReset(em); toast("Password reset email sent"); }
    catch(err){ const el=$("acctErr"); if(el) el.textContent=acctErrMsg(err); }
    return;
  }
  if(a==="acct-submit"){
    const em=($("acctEmail")?.value||"").trim();
    const pw=($("acctPass")?.value||"");
    const el=$("acctErr"); if(el) el.textContent="";
    if(!em || em.indexOf("@")<0){ if(el) el.textContent="Enter a valid email address."; return; }
    if(pw.length<6){ if(el) el.textContent="Password must be at least 6 characters."; return; }
    t.disabled=true; const lbl=t.textContent; t.textContent="Working…";
    try{
      if((App.acctMode||"signup")==="signup") await accountSignUp(em,pw);
      else await accountSignIn(em,pw);
      App.screen="home"; render(); toast("Signed in — progress is syncing ✓");
    }catch(err){
      t.disabled=false; t.textContent=lbl;
      const e2=$("acctErr"); if(e2) e2.textContent=acctErrMsg(err);
    }
    return;
  }

  if(a==="nav"){ const sc=t.dataset.screen; if(sc==="exam-builder") App.builder=null; if(sc==="account"&&t.dataset.mode) App.acctMode=t.dataset.mode; App.screen=sc; render(); if(sc==="leaderboard") syncBoard(); return; }
  ...
  if(a==="set-srsgrading"){ DB.settings.srsGrading=(DB.settings.srsGrading===false); save.settings(); render(); return; }
  ...
  if(a==="open-bank-system"){ App.nav={system:t.dataset.system, type:null, reference:null}; App.screen="banksys"; render(); return; }
  if(a==="open-bank-type"){ App.nav.type=t.dataset.type; bankPickSet(App.nav.system, App.nav.type); App.screen="banktype"; render(); window.scrollTo({top:0,behavior:"instant"}); return; }
  if(a==="bank-pick"){ const r=t.dataset.ref, refs=bankPickSet(App.nav.system, App.nav.type); if(refs.has(r)) refs.delete(r); else refs.add(r); render(); return; }
  if(a==="bank-start"){ const bp=App.bankPick; if(!bp||!bp.refs.size){ toast("Tick at least one session"); return; }
    const ids=[]; bp.refs.forEach(r=>poolFor({system:bp.system,type:bp.type,reference:r}).forEach(id=>ids.push(id)));
    const uniq=[...new Set(ids)]; if(!uniq.length){ toast("No questions in the selected sessions"); return; }
    startPracticeCtx({ids:uniq}, bp.system+" · "+bp.type+" · "+bp.refs.size+" session"+(bp.refs.size>1?"s":"")); return; }
...
async function checkNewReports(){
  ...
  try{
    ...
    const r=await fetch(u,{cache:"no-store"}); if(!r.ok) return;
    const data=await r.json();
    if(!Array.isArray(data) || (data.length && !Array.isArray(data[0]))) return;   // not upgraded / unauthorized
    App.inboxRows=data;
...
export async function boot(){
  ...
  try{ adminToken = await wsGet("medrecall:admintoken:v1"); }catch(e){}
  // heal devices that cached leaderboard rows as notices before the shape guard
  if(Array.isArray(DB.settings.remoteAnnounce)){
    const kept = DB.settings.remoteAnnounce.filter(isAnnouncement);
    if(kept.length !== DB.settings.remoteAnnounce.length){ DB.settings.remoteAnnounce = kept; save.settings(); }
  }
  ...
  buildIndex();
  // first launch: let the user choose an account or guest mode before anything else
  if(!DB.settings.onboarded && App.screen==="home") App.screen="welcome";
  render();
  if(isSignedIn()) syncNow(true);
  ...
  fetchEdits(); fetchAnnouncements(); if(DB.settings.maintainer) checkNewReports();
...
export function resetDB() {
  ...
  DB.exams = [];
  DB.reports = [];
  DB.settings = { onboarded:false, account:{mode:"",uid:"",email:"",lastSync:"",autoSync:true}, announcements:[], remoteAnnounce:[], seenAnnounce:[], newPerDay: 20, passMark: 50, maintainer: false, wallpaper: "ink", theme: "dark", dailyGoal: 20, sounds: true, examDate: "", revealOnPick: true, srsGrading: true, notif: { enabled:false, daily:true, due:true, streak:true, exam:true, time:"19:00", lastFired:{} } };
}
/* clinic-v2.5 */
