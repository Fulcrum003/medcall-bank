...
export async function syncNow(quiet){
  ...
  try{
    ...
    if(snap.exists()){
      ...
      DB.progress = mergeProgress(DB.progress, remote.progress||{});
      DB.exams    = mergeRecords(DB.exams, remote.exams, "id");
      if(remote.settings) SYNCED_SETTINGS.forEach(k=>{
        if(remote.settings[k]!==undefined && DB.settings[k]===undefined) DB.settings[k]=remote.settings[k];
      });
      save.progress(); save.exams();
    }
    let payload = buildSyncPayload(DB, 20);
    if(JSON.stringify(payload).length > 800000) payload = buildSyncPayload(DB, 6);
...
export function pendingAnnounce(){
  ...
  return all.filter(a => a && a.id && seen.indexOf(a.id) < 0)
...
export function announceHistory(){
  ...
  const rows = all.slice().sort((a,b)=> String(b.date||"").localeCompare(String(a.date||"")))
...
export async function fetchAnnouncements(){
  ...
  try{
    ...
    const data = await r.json();
    if(Array.isArray(data)){ DB.settings.remoteAnnounce = data; save.settings(); }
...
export function announceComposer(){
  ...
  const rows = live.slice().reverse().map(a=>`<div class="row between" style="padding:9px 0;border-top:1px solid var(--line-soft)">
  ...
  <div class="card pad">
    ...
    ${live.length?`<div style="margin-top:12px"><div class="faint" style="font-size:12px;margin-bottom:2px">Live now (${live.length})</div>${rows}</div>`:""}
  </div>`;
}
