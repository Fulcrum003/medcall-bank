...
export let DB = {
  progress:{ questions:{}, resume:null, streak:{current:0,lastStudied:null} },
  exams:[],
  reports:[],
  settings:{ newPerDay:20, passMark:50, maintainer:false, wallpaper:"ink", theme:"dark", dailyGoal:20, sounds:false }
};
...
/* ============================================================
   APP STATE
   ============================================================ */
export const App = { screen:"home", nav:{system:null,reference:null}, practice:null, exam:null, examResult:null, builder:null, examReview:false };
...
const listReferences = sys     => groupCounts(q=>qSys(q)===sys, qRef);
const listTopics     = (sys,r) => groupCounts(q=>qSys(q)===sys && qRef(q)===r, q=>q.topic||"General");
const poolFor = ctx => {
  ...
  sys.forEach(s=>{
    ...
    html+=`<button class="card pad subj" data-action="open-system" data-system="${esc(s.name)}" style="margin-bottom:10px">
      ...
      <div class="row" style="gap:14px;margin-top:7px;font-size:12.5px;color:var(--faint)">
        <span>${s.total} questions</span><span>·</span><span>${listReferences(s.name).length} references</span>
      </div>
...
function viewSystem(){
  const sys=App.nav.system, refs=listReferences(sys);
  ...
  let html=`<div class="fade">
    ...
    <p class="muted" style="font-size:13.5px;margin-top:2px">${refs.length} references · ${poolFor({system:sys}).length} questions</p>
    <div style="height:10px"></div>
  ...
  refs.forEach(r=>{
    ...
    html+=`<button class="card pad subj" data-action="open-reference" data-system="${esc(sys)}" data-reference="${esc(r.name)}" style="margin-bottom:10px">
      ...
      <div class="row" style="gap:14px;margin-top:7px;font-size:12.5px;color:var(--faint)"><span>${r.total} questions</span><span>·</span><span>${listTopics(sys,r.name).length} topics</span></div>
      <div class="progressbar"><i style="width:${pct}%"></i></div>
    </button>`;
  });
  html+=`</div>`;
...
function viewReference(){
  const sys=App.nav.system, ref=App.nav.reference, tops=listTopics(sys,ref);
  let html=`<div class="fade">
    ...
    <div style="height:10px"></div>
    <button class="btn btn-primary" data-action="study-reference" data-system="${esc(sys)}" data-reference="${esc(ref)}">Study all of this reference</button>
    <div class="sectlabel">Topics</div>`;
  tops.forEach(tp=>{
    const pct=tp.total?Math.round(tp.seen/tp.total*100):0;
    html+=`<button class="card pad subj" data-action="study-topic" data-system="${esc(sys)}" data-reference="${esc(ref)}" data-topic="${esc(tp.name)}" style="margin-bottom:9px">
...
function viewQuiz(){
  ...
  if(q.type==="sa"){
    ...
    if(q.type==="emq") html+=`<div class="optshdr">Pick the single best option</div>`;
  ...
  let foot;
  if(!s.revealed){
    foot=`<button class="btn btn-primary" data-action="reveal" ${(q.type==="sa"||s.selected)?'':'disabled'}>Reveal answer</button>`;
  } else {
...
function advancePractice(){
  ...
  s.i++; s.revealed=false; s.selected=null; s.saText="";
...
document.body.addEventListener("click", async e=>{
  ...
  if(a==="open-system"){ App.nav={system:t.dataset.system, reference:null}; App.screen="system"; render(); return; }
  if(a==="open-reference"){ App.nav={system:t.dataset.system, reference:t.dataset.reference}; App.screen="reference"; render(); return; }
  if(a==="study-system"){ startPracticeCtx({system:t.dataset.system}, t.dataset.system); return; }
  if(a==="study-reference"){ startPracticeCtx({system:t.dataset.system, reference:t.dataset.reference}, t.dataset.reference); return; }
  if(a==="study-topic"){ startPracticeCtx({system:t.dataset.system, reference:t.dataset.reference, topic:t.dataset.topic}, t.dataset.topic); return; }
...
export function resetDB() {
  DB.progress = { questions: {}, resume: null, streak: { current: 0, lastStudied: null } };
  DB.exams = [];
  DB.reports = [];
  DB.settings = { newPerDay: 20, passMark: 50, maintainer: false, wallpaper: "ink", theme: "dark", dailyGoal: 20, sounds: false };
}
